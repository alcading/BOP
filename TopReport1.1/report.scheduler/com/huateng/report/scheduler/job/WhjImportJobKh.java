...
import com.huateng.report.imports.batch.WhjImportJobTaskKh;
 ...
 * 生成定时任务
 */
public class WhjImportJobKh implements Job {
	...
	public void execute(JobExecutionContext context) throws JobExecutionException {
		...
		log.info("####tianqf####开始执行客户信息入库定时任务");
		if ("ON".equalsIgnoreCase(onOff)) {
			...
			try {
				...
				if (isdo) {
					...
					log.info("==222定时客户信息入库begin==");
					/*ImportJobTask importJobTask = new ImportJobTask();
					importJobTask.doBiz();*/
					WhjImportJobTaskKh whjImportJobTaskKh = new WhjImportJobTaskKh();
					whjImportJobTaskKh.doBiz();
					log.info("==222定时客户信息入库end==");
					endTm = new Date();
