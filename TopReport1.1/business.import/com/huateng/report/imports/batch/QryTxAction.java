 ...
 */
public class QryTxAction extends com.huateng.mvc.struts.action.BaseAction {

	private static SysDicStrMethod sysDic = new SysDicStrMethod();
	private static final Logger logger = Logger.getLogger(QryTxAction.class); 
...
//		String els = request.getParameter(cqId+"_"+QueryExportForm.P_COL_SORT_NAME);
		...
		logger.debug("登录机构："+brno);
		
		//此处注释掉的代码用于生成手动填报的excel
//		WhjBranchInfoService whjBranchInfoService = WhjBranchInfoService.getInstance();
//		List<BaseWhjBranchInfo> branchInfoList = whjBranchInfoService.getWhjBranchInfo(brno);
//		BaseWhjBranchInfo branchInfo = branchInfoList.get(0);
		StringBuffer name = new StringBuffer();
//		if("TX".equals(fileName)){
//			name.append("CRDTA");		
//			name.append(branchInfo.getWhjjgdm());
//			SimpleDateFormat sdf1 = new SimpleDateFormat("yyMMdd");
//			String date = sdf1.format(new Date());
//			name.append(date);
//			name.append("00");
//		}else if("XF".equals(fileName)){
//			name.append("CRXXA");		
//			name.append(branchInfo.getWhjjgdm());
//			SimpleDateFormat sdf1 = new SimpleDateFormat("yyMMdd");
//			String date = sdf1.format(new Date());
...
//		}
		...
		form.setFileName(name.toString());
	...
	public String proBatch(QueryExportForm form) {
		...
		logger.debug("####结束页码:"+form.getEndPage());
		//起始页码
		if(form.getEndPage()!=null){
