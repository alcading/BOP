...
import com.huateng.report.dataquery.getter.WhjTxQueryGetter;
import com.huateng.service.pub.RoleMgrService;

public class CQMethod {

	
	private static final Logger logger = Logger.getLogger(WhjTxQueryGetter.class);
	/**
...
//	}
	 ...
	 * @throws HuatengException
	 */
	public static String getJgmcByJgbs(ICommonQueryBaseBean element,
			String value, ServletRequest request) throws HuatengException {
		...
		ROOTDAO rootdao = ROOTDAOUtils.getROOTDAO();
		List l = rootdao.queryByQL2List("from Bctl bctl where  bctl.brno = '" + value + "'");
		if (l == null || l.isEmpty()) {
			return "";
		} else {
			Bctl bctl = (Bctl) l.get(0);
