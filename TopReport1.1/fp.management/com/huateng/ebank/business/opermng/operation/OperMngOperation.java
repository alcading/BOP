			...
			if (!tls.isNeedApprove(ReportEnum.REPORT_TASK_FUNCID.TASK_100399.value)) {
				...
				if (tlrInfoDAO.query(tlrInfo.getTlrno()) == null) {
					...
					int id1 = 0;
						...
						Iterator it1 = DAOUtils.getHQLDAO().queryByQL("select max(id) from TlrRoleRel");
						if (it1.hasNext()) {
							Number num = (Number) it1.next();
							id1 = num.intValue() + 1;
						}
					...
					for(Bctl bc : bctls){
						TlrBctlRel tlrBctlRel = new TlrBctlRel(id);
					...
					for(RoleInfo rl : roles){
						TlrRoleRel tlrRoleRel = new TlrRoleRel(id1);
					...
					for(TlrMngRelBean abc : tlrs){
						TlrManRel tlrrel=new TlrManRel(10);
